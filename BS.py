...
class BS:
    def __init__(self, idBS: int, coord):
        ...
        self.nbNode = 21
        self.agent = DQNAgent(gamma=0, epsilon=1, lr=0.00008, n_actions=len(self.combinations), input_dims=(4,),
    ...
    def recommandation(self, param1, param2, node, reward):
        done = False
        param1carre = np.sin(param1)
        param2carre = np.sin(param2)
        product = param1 * param2
        state = np.array([param1carre, param2carre, param1, param2])
        if self.first[node]:
