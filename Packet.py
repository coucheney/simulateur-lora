...
class Packet:
    ...
    coordNode : coordonées de la node ayant envoyé le paquet
    nbSend : nombre de fois ou le même paquet à été retransmit
    rssi : puissance du packet au niveau de l'antenne
    freq : fréquence utilisée pour envoyé pour envoyer le paquet
    sendDate : date à laquelle le paquet commence à être envoyé, prend une valeurs lors de l'envoi du paquet
    power : puissance utilisée pour envoyer le paquets
    energyCost : cout energétique de l'envoie
    """
    def __init__(self, nodeId: int, packetLen: int, sf: int, cr: int, bw: int, pNode: Point, power: int, TX, packetId, freq):
        self.nodeId = nodeId
